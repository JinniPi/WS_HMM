...
from word_recognition.article import Article
from word_recognition.helper import Helper
import threading
import time
import math
...
class HiddenMarkovModel(object):
    def __init__(self, states, observations, transitions, emissions,
        ...
        self.transitions = transitions # ma trận chuyển
        self.emissions = emissions # ma trận sinh
    ...
    def baum_welch_algorithm(self, list_observations_sequence, number_thread):
        ...
        :param list_observations_sequence: each obervations_sequence coverted to number
        :param number_thread:
        :return:
        """
...
if __name__ == '__main__':
    ...
    for index, doc in enumerate(doc_array):
        print 'Prepare training in doc %i' % (index)
    ...
    print 'len symbol %i' % (len(symbol_appeared))
    print 'symbol %s ' % symbol_appeared
    start = time.clock()
